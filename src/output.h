...
{
    return fold_and_print(w, begin, width, base_color, string_format(mes, std::forward<Args>(args)...));
}
/**
...
template <typename TabList, typename CurrentTab, typename = std::enable_if_t<std::is_same<CurrentTab, std::remove_const_t<typename TabList::value_type::first_type>>::value>>
...
template <typename TabList, typename TabKeys, typename CurrentTab, typename = std::enable_if_t<std::is_same<CurrentTab, std::remove_const_t<typename TabList::value_type::first_type>>::value>>
