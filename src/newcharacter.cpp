...
tab_direction set_stats(avatar &u, points_left &points)
{
    ...
    ui.on_redraw([&](const ui_adaptor &) {
        ...
        case 3:
        {
            ...
            mvwprintz(w_description, point_zero, (read_spd == 100 ? COL_STAT_NEUTRAL : (read_spd < 100 ? COL_STAT_BONUS : COL_STAT_PENALTY)),
...
tab_direction set_traits(avatar &u, points_left &points)
{
    ...
    ui.on_redraw([&](const ui_adaptor &) {
        ...
        for (int iCurrentPage = 0; iCurrentPage < 3; iCurrentPage++)
        {
            ...
            for (int i = start; i < end; i++)
            {
                ...
                mvwprintz(w, point(cur_line_x, cur_line_y), cLine, utf8_truncate(mdata.name(), page_width - 2));
            }
    ...
    do
    {
        ...
        else if (action == "CONFIRM")
        {
            ...
            else if (iCurWorkingPage == 0 && num_good + mdata.points > max_trait_points && !points.is_freeform())
            {
            ...
            else if (iCurWorkingPage != 0 && num_bad + mdata.points < -max_trait_points && !points.is_freeform())
            {
                             ...
                             const tab_direction direction)
{
    ...
    ui.on_redraw([&](const ui_adaptor &) {
        ...
        const int end_pos = iStartPos + ((iContentHeight > profs_length) ? profs_length : iContentHeight);
        int i;
                           ...
                           const tab_direction direction)
{
    ...
    ui.on_redraw([&](const ui_adaptor &) {
        ...
        const int end_pos = iStartPos + ((iContentHeight > scens_length) ? scens_length : iContentHeight);
        int i;
        for (i = iStartPos; i < end_pos; i++)
        {
            mvwprintz(w, point(2, 5 + i - iStartPos), c_light_gray,
            ...
            {
                col = (cur_id_is_valid &&
                               sorted_scens[i] == sorted_scens[cur_id]
                           ? hilite(COL_SKILL_USED)
                           : COL_SKILL_USED);
            }
            mvwprintz(w, point(2, 5 + i - iStartPos), col,
...
namespace char_creation
{
                            ...
                            const bool highlight)
    {
        ...
        mvwprintz(w_height, point(height_pos, 0), c_white, string_format("%d cm", you.base_height()));
                              ...
                              const bool highlight)
    {
        ...
        mvwprintz(w_location, point(utf8_width(_("Starting location:")) + 1, 0),
                  ...
                  you.random_start_location ? remove_color_tags(random_start_location_text) : string_format(ngettext("%s (%d variant)", "%s (%d variants)", you.start_location.obj().targets_count()), you.start_location.obj().name(), you.start_location.obj().targets_count()));
        wnoutrefresh(w_location);
    }
                              ...
                              points_left &points)
{
    ...
    ui.on_redraw([&](const ui_adaptor &) {
        ...
        {
            pos = (utf8_width(vStatNames[i]) > pos ? utf8_width(vStatNames[i]) : pos);
        ...
        if (isWide)
        {
            ...
            std::vector<trait_id> current_traits = points.limit == points_left::TRANSFER ? you.get_mutations() : you.get_base_traits();
        ...
        if (isWide)
        {
            ...
            if (!MAP_SHARING::isSharing() && allow_reroll)
            { // no random names when sharing maps
    ...
    do
    {
        ...
        {
            if (!MAP_SHARING::isSharing())
            { // Don't allow random names when sharing maps. We don't need to check at the top as you won't be able to edit the name
                you.pick_name();
...
void avatar::save_template(const std::string &name, const points_left &points)
{
    ...
    if (native.find_first_of("\"*/:<>?\\|"
                             ...
                             "\x1C\x1D\x1E\x1F") != std::string::npos)
    {
    ...
    write_to_file(
        PATH_INFO::templatedir() + native + ".template", [&](std::ostream &fout) {
