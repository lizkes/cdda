...
namespace io
{
    ...
    std::string enum_to_string<blood_type>(blood_type data)
    {
        ...
        {
            // *INDENT-OFF*
        case blood_type::blood_O:
            return "O";
        case blood_type::blood_A:
            return "A";
        case blood_type::blood_B:
            return "B";
        case blood_type::blood_AB:
            return "AB";
            // *INDENT-ON*
...
// *INDENT-OFF*
Character::Character() : cached_time(calendar::before_time_starts),
                         id(-1),
...
void Character::randomize_blood()
{
    ...
    static const std::array<std::tuple<double, blood_type, bool>, 8> blood_type_distribution = {{std::make_tuple(0.3821, blood_type::blood_O, true),
...
int Character::sight_range(int light_level) const
{
     ...
     */
    int range = static_cast<int>(-std::log(get_vision_threshold(static_cast<int>(get_map().ambient_light_at(pos()))) / static_cast<float>(light_level)) *
...
int Character::swim_speed() const
{
    ...
    // base swim speed.
    ret = (440 * mutation_value("movecost_swim_modifier")) + weight_carried() / (60_gram / mutation_value("movecost_swim_modifier")) - 50 * get_skill_level(skill_swimming);
...
bool Character::check_outbounds_activity(const player_activity &act, bool check_only)
{
    ...
    if ((act.placement != tripoint_zero && act.placement != tripoint_min &&
         !here.inbounds(here.getlocal(act.placement))) ||
        (!act.coords.empty() &&
...
void Character::recalc_sight_limits()
{
    ...
    if (has_active_bionic(bio_infrared) ||
        ...
        worn_with_flag(flag_IR_EFFECT) || (is_mounted() && mounted_creature->has_flag(MF_MECH_RECON_VISION)))
    {
...
void Character::practice(const skill_id &id, int amount, int cap, bool suppress_warning)
{
    ...
    if (amount > 0 && get_skill_level(id) > cap)
    { //blunt grinding cap implementation for crafting
        amount = 0;
        if (!suppress_warning)
        {
        ...
        for (const bodypart_id &bp : get_all_body_parts())
        {
            if (to_wear.covers(bp) && encumb(bp) >= 40)
            {
                add_msg_if_player(m_warning,
                                  bp == body_part_eyes ? _("Your %s are very encumbered!  %s") : _("Your %s is very encumbered!  %s"),
...
item *Character::invlet_to_item(const int linvlet)
{
    ...
    if (is_npc())
    {
        DebugLog(D_WARNING, D_GAME) << "Why do you need to call Character::invlet_to_position on npc " << name;
    }
...
units::mass Character::weight_carried_with_tweaks(const item_tweaks &tweaks) const
{
    ...
    const std::map<const item *, int> &without = tweaks.without_items ? tweaks.without_items->get() : empty;
...
units::volume Character::volume_carried_with_tweaks(const std::vector<std::pair<item_location, int>>
                                                        &locations) const
{
    std::map<const item *, int> dropping;
    for (const std::pair<item_location, int> &location_pair : locations)
...
ret_val<bool> Character::can_wear(const item &it, bool with_equip_change) const
{
    ...
    if (!it.has_flag(flag_OVERSIZE) && !it.has_flag(flag_SEMITANGIBLE))
    {
        for (const trait_id &mut : get_mutations())
        {
            ...
            {
                return ret_val<bool>::make_failure(is_player() ? _("Your %s mutation prevents you from wearing your %s.") : _("My %s mutation prevents me from wearing this %s."), branch.name(),
                                                   it.type_name());
            }
        }
             ...
             has_trait(trait_ANTLERS)))
        {
            return ret_val<bool>::make_failure(_("Cannot wear a helmet over %s."),
                                               (has_trait(trait_HORNS_POINTED) ? _("horns") : (has_trait(trait_ANTENNAE) ? _("antennae") : _("antlers"))));
        }
    }
    ...
    if (it.has_flag(flag_SPLINT))
    {
        ...
        if (!need_splint)
        {
            return ret_val<bool>::make_failure(is_player() ? _("You don't have any broken limbs this could help.")
...
int Character::rust_rate() const
{
    ...
    int ret = ((rate_option == "vanilla" || rate_option == "capped") ? 100 : 100 + 10 * (8 - intel));
                       ...
                       std::map<bodypart_id, layer_level> &highest_layer_so_far, bool power_armor, const Character &c)
{
    ...
    for (const bodypart_id &bp : c.get_all_body_parts())
    {
        ...
        const int armorenc = !power_armor || !it.is_power_armor() ? encumber_val : std::max(0, encumber_val - 40);
...
bool Character::in_climate_control()
{
    ...
    if (calendar::turn >= next_climate_control_check)
    {
        ...
        if (const optional_vpart_position vp = here.veh_at(pos()))
        {
            ...
            regulated_area = (vp->is_inside() &&                    // Already checks for opened doors
...
int Character::get_speed() const
{
    ...
    {
        return get_speed_base() + std::max(0, get_speed_bonus()) + std::max(0, get_speedydex_bonus(get_dex()));
    }
...
{
    static const std::vector<std::pair<float, float>> starv_thresholds = {{std::make_pair(0.0f, 6000.0f),
...
std::pair<std::string, nc_color> Character::get_hunger_description() const
{
    ...
    static const std::array<std::tuple<const efftype_id &, const char *, nc_color>, 9> hunger_states{{std::forward_as_tuple(effect_hunger_engorged, translate_marker("Engorged"), c_red),
...
bool Character::is_deaf() const
{
    return get_effect_int(effect_deaf) > 2 || worn_with_flag(flag_DEAF) ||
           ...
           (has_trait(trait_M_SKIN3) && get_map().has_flag_ter_or_furn("FUNGUS", pos()) && in_sleep_state());
}
...
int Character::get_max_healthy() const
{
    ...
    return clamp(static_cast<int>(std::round(-3 * (bmi - character_weight_category::normal) *
                                                 (bmi - character_weight_category::overweight) +
                                             200)),
                 -200, 200);
}
...
{
    return (to_turn<int>(to) / to_turns<int>(tick_length)) - (to_turn<int>(from) / to_turns<int>(tick_length));
}
...
void Character::check_needs_extremes()
{
    ...
    if (is_player() && get_thirst() >= 600 && (stomach.get_water() == 0_ml || guts.get_water() == 0_ml))
    {
        if (get_thirst() >= 1200)
        {
    ...
    if (sleep_deprivation >= SLEEP_DEPRIVATION_HARMLESS && !in_sleep_state())
    {
        if (calendar::once_every(60_minutes))
        {
            ...
            if (can_pass_out && calendar::once_every(10_minutes))
            {
                ...
                {
                    add_msg_player_or_npc(m_bad,
                                          _("Your body collapses due to sleep deprivation, your neglected fatigue rushing back all at once, and you pass out on the spot."), _("<npcname> collapses to the ground from exhaustion."));
...
void Character::update_bodytemp()
{
    ...
    // Fatigue
    // ~-900 when exhausted
    const int fatigue_warmth = has_sleep ? 0 : static_cast<int>(std::min(0.0f, -1.5f * get_fatigue()));

    // Sunlight
    const int sunlight_warmth = g->is_in_sunlight(pos()) ? (get_weather().weather_id->sun_intensity ==
    ...
    for (const bodypart_id &bp : get_all_body_parts())
    {
        ...
        blister_count += h_radiation - 111 > 0 ? std::max(static_cast<int>(std::sqrt(h_radiation - 111)), 0) : 0;
        ...
        else if (pyromania && best_fire >= 1)
        { // Only give us fire bonus if there's actually fire
                                   ...
                                   int bleed, float bite, float infect, float bandage_power, float disinfectant_power) const
{
    ...
    for (size_t i = 0; i < parts.size(); i++)
    {
        ...
        if (bleeding)
        {
            desc += colorize(string_format("%s: %s", get_effect(effect_bleed, bp).get_speed_name(),
                                           get_effect(effect_bleed, bp).disp_short_desc()),
                             c_red) +
                    "\n";
            if (bleed > 0)
            {
    ...
    if (!is_valid_choice)
    { // no body part can be chosen for this item/effect
        bmenu.init();
...
int Character::throw_range(const item &it) const
{
    ...
    int ret = (str_override * 10) / (tmp.weight() >= 150_gram ? tmp.weight() / 113_gram : 10 - static_cast<int>(tmp.weight() / 15_gram));
...
float Character::healing_rate_medicine(float at_rest_quality, const bodypart_id &bp) const
{
    ...
    for (const auto &elem : *effects)
    {
        for (const std::pair<const bodypart_id, effect> &i : elem.second)
        {
            ...
            float tmp_rate = static_cast<float>(eff.get_amount("HEAL_RATE")) / to_turns<int>(24_hours);
...
bool Character::dispose_item(item_location &&obj, const std::string &prompt)
{
    ...
    for (auto &e : worn)
    {
        ...
        {
            const holster_actor *ptr = dynamic_cast<const holster_actor *>(e.type->get_use("holster")->get_actor_ptr());
            opts.emplace_back(dispose_option{
    ...
    for (const auto &e : opts)
    {
        menu.addentry(-1, e.enabled, e.invlet, string_format(e.enabled ? "%s | %-7d" : "%s |", e.prompt, e.moves));
    }
...
void Character::vomit()
{
    ...
    if (!has_effect(effect_nausea))
    { // Prevents never-ending nausea
        const effect dummy_nausea(effect_source(this), &effect_nausea.obj(), 0_turns,
                                  bodypart_str_id::NULL_ID(), false, 1, calendar::turn);
        add_effect(effect_nausea, std::max(dummy_nausea.get_max_duration() * units::to_milliliter(stomach.contains()) / 21, dummy_nausea.get_int_dur_factor()));
    }
...
bool Character::armor_absorb(damage_unit &du, item &armor, const bodypart_id &bp)
{
    ...
    std::string damage_verb = (du.type == damage_type::BASH) ? material.bash_dmg_verb() : material.cut_dmg_verb();

    const std::string pre_damage_name = armor.tname();
    const std::string pre_damage_adj = armor.get_base_material().dmg_adj(armor.damage_level());

    // add "further" if the damage adjective and verb are the same
    std::string format_string = (pre_damage_adj == damage_verb) ? _("Your %1$s is %2$s further!") : _("Your %1$s is %2$s!");
    ...
    return armor.mod_damage(armor.has_flag(flag_FRAGILE) ? rng(2 * itype::damage_scale, 3 * itype::damage_scale) : itype::damage_scale, du.type);
}
...
units::volume Character::volume_capacity_with_tweaks(const std::vector<std::pair<item_location, int>>
                                                         &locations) const
{
    std::map<const item *, int> dropping;
    for (const std::pair<item_location, int> &location_pair : locations)
                ...
                if (here.impassable(point) &&
                    !(weapon.has_flag(flag_SPEAR) && // Fences etc. Spears can stab through those
                      here.has_flag(STATIC("THIN_OBSTACLE"),
                                    point)))
                { //this mirrors melee.cpp function reach_attack
                    can_see = false;
                    break;
                }
            }
...
std::vector<Creature *> Character::get_hostile_creatures(int range) const
{
    return g->get_creatures_if([this, range](const Creature &critter) -> bool {
        ...
        return this != &critter && pos() != critter.pos() && // TODO: get rid of fake npcs (pos() check)
               dist_to_creature <= range && critter.attitude_to(*this) == Creature::Attitude::HOSTILE && sees(critter);
    });
}
