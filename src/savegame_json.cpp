...
void Character::load(const JsonObject &data)
{
    ...
    if (!backlog.empty() && !backlog.front().str_values.empty() && ((activity && activity.id() == activity_id("ACT_FETCH_REQUIRED")) || (destination_activity && destination_activity.id() == activity_id("ACT_FETCH_REQUIRED"))))
    {
...
void avatar::load(const JsonObject &data)
{
    ...
    grab(iter == obj_type_name.end() ? object_type::NONE : static_cast<object_type>(std::distance(obj_type_name.begin(), iter)),
         grab_point);
...
void npc::load(const JsonObject &data)
{
    ...
    if (data.read("mission", misstmp))
    {
        ...
        static const std::set<npc_mission> legacy_missions = {{NPC_MISSION_LEGACY_1, NPC_MISSION_LEGACY_2,
    ...
    if (data.read("attitude", atttmp))
    {
        ...
        static const std::set<npc_attitude> legacy_attitudes = {{NPCATT_LEGACY_1, NPCATT_LEGACY_2, NPCATT_LEGACY_3,
    ...
    if (data.has_array("sp_timeout"))
    {
        ...
        while (parray.has_more() && index < type->special_attacks_names.size())
        {
            if (parray.read_next(ptimeout))
            {
                ...
                { // -1 means disabled, unclear what <-1 values mean in old saves
...
void item::io(Archive &archive)
{
    ...
    archive.template io<const itype>(
        "typeid", type, load_type, [](const itype &i) {
            return i.get_id().str();
        },
        io::required_tag());
...
void item::migrate_content_item(const item &contained)
{
    ...
    else if (!contained.made_of(phase_id::LIQUID) && (contained.is_magazine() || contained.is_ammo()))
    {
...
void item::deserialize(JsonIn &jsin)
{
    ...
    { // empty contents was not serialized, recreate pockets from the type
