...
void pocket_favorite_callback::refresh(uilist *menu)
{
    ...
    if (selected_pocket != nullptr)
    {
        ...
        fold_and_print(menu->window, point(2, 2), width,
                       c_light_gray, string_format(_("Press a key to add to %s"), colorize(whitelist ? _("whitelist") : _("blacklist"), c_light_blue)));
...
{
    return colorize("p", c_light_green) + _(" priority, ") +
                                   ...
                                   uilist *menu)
{
    ...
    else if (cat_id)
    {
        ...
        std::sort(all_cat.begin(), all_cat.end(), [](const item_category &lhs, const item_category &rhs) {
...
bool item_contents::full(bool allow_bucket) const
{
    for (const item_pocket &pocket : contents)
    {
        if (pocket.is_type(item_pocket::pocket_type::CONTAINER) && !pocket.full(allow_bucket))
        {
            return false;
        }
    }
...
bool item_contents::will_spill_if_unsealed() const
{
    for (const item_pocket &pocket : contents)
    {
        if (pocket.is_type(item_pocket::pocket_type::CONTAINER) && pocket.will_spill_if_unsealed())
        {
            return true;
        }
    }
...
units::volume item_contents::get_nested_content_volume_recursive(const std::map<const item *, int> &without) const
{
    units::volume ret = 0_ml;
