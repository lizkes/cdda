...
static void WinCreate()
{
    ...
    if (!software_renderer)
    {
        ...
        renderer.reset(SDL_CreateRenderer(::window.get(), renderer_id, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC | SDL_RENDERER_TARGETTEXTURE));
...
void cata_cursesport::curses_drawwindow(const catacurses::window &w)
{
    ...
    if (g && w == g->w_terrain && use_tiles)
    {
        ...
        for (const auto &iter : overlay_strings)
        {
            ...
            for (size_t i = 0; i < text.size(); ++i)
            {
                ...
                if (x < x0 || x > x0 + (TERRAIN_WINDOW_TERM_WIDTH - 1) * font->width || y < y0 || y > y0 + (TERRAIN_WINDOW_TERM_HEIGHT - 1) * font->height)
                {
                    continue;
                }
    ...
    else if (g && w == g->w_terrain && map_font)
    {
        ...
        if (partial_height > 0)
        {
            geometry->rect(renderer, point(win->pos.x * map_font->width, (win->pos.y + TERRAIN_WINDOW_HEIGHT) * map_font->height),
        ...
        if (partial_width > 0)
        {
            geometry->rect(renderer, point((win->pos.x + TERRAIN_WINDOW_WIDTH) * map_font->width, win->pos.y * map_font->height),
                           partial_width,
...
{
    return (action == "UP" || action == "DOWN" || action == "LEFT" || action == "RIGHT" || action == "LEFTUP" || action == "LEFTDOWN" || action == "RIGHTUP" || action == "RIGHTDOWN" || action == "QUIT" || action == "CONFIRM" || action == "MOVE_SINGLE_ITEM" // maps to ENTER
            || action == "MOVE_ARMOR"                                                                                                                                                                                                                            // maps to ENTER
            || action == "ANY_INPUT" || action == "DELETE_TEMPLATE"                                                                                                                                                                                              // strictly we shouldn't have this one, but I don't like seeing the "d" on the main menu by default. :)
    );
}
...
void draw_quick_shortcuts()
{
    ...
    { // player is swipe + holding in a direction
        return;
    }
    ...
    for (std::list<input_event>::iterator it = qsl.begin(); it != qsl.end(); ++it)
    {
        ...
        show_hint = hovered &&
                    SDL_GetTicks() - finger_down_time > static_cast<uint32_t>(get_option<int>("ANDROID_INITIAL_DELAY"));
        ...
        if (shortcut_right)
        {
            text_x = (WindowWidth - (i + 0.5f) * width - (font->width * utf8_width(text)) * text_scale * 0.5f) / text_scale;
        }
        else
        {
...
void draw_virtual_joystick()
{
    ...
    dstrect.w = dstrect.h = (get_option<float>("ANDROID_DEADZONE_RANGE") +
                             get_option<float>("ANDROID_REPEAT_DELAY_RANGE")) *
                            longest_window_edge * 2;
    dstrect.x = finger_down_x - dstrect.w / 2;
    dstrect.y = finger_down_y - dstrect.h / 2;
    RenderCopy(renderer, touch_joystick, NULL, &dstrect);
...
bool is_string_input(input_context &ctx)
{
    ...
    return category == "STRING_INPUT" || category == "HELP_KEYBINDINGS" || category == "NEW_CHAR_DESCRIPTION" || category == "WORLDGEN_CONFIRM_DIALOG";
}
...
void handle_finger_input(uint32_t ticks)
{
    ...
    {
        if (ticks - finger_down_time >= static_cast<uint32_t>(get_option<int>("ANDROID_INITIAL_DELAY")))
        {
...
static void CheckMessages()
{
    ...
    if (!needupdate)
    {
                    ...
                    for (int dy = -1; dy <= 1; dy++)
                    {
                        ...
                        // Check if we're near a vehicle, if so, vehicle controls should be top.
                        {
                            ...
                            if (veh)
                            {
                                ...
                                if (openablepart >= 0 && veh->is_open(openablepart) && (dx != 0 || dy != 0))
                                { // an open door adjacent to us
        ...
        if (ac_back_down_time > 0 &&
            ticks - ac_back_down_time > static_cast<uint32_t>(get_option<int>("ANDROID_INITIAL_DELAY")))
        {
    ...
    while (SDL_PollEvent(&ev))
    {
        ...
        case SDL_KEYUP:
        {
            ...
            {
                if (ticks - ac_back_down_time <= static_cast<uint32_t>(get_option<int>("ANDROID_INITIAL_DELAY")))
                {
                    if (SDL_IsTextInputActive())
                    {
                        SDL_StopTextInput();
                        ...
                        if (is_default_mode)
                        {
                            ...
                            if (std::max(d1, d2) < get_option<float>("ANDROID_DEADZONE_RANGE") * longest_window_edge)
                            {
                                last_input = input_event(get_key_event_from_string(
                                                             get_option<std::string>("ANDROID_2_TAP_KEY")),
                                                         input_event_t::keyboard_char);
                            }
                            else
                            {
                                ...
                                if (dot > 0.0f)
                                { // both fingers mostly heading in same direction, check for double-finger swipe gesture
                                    float dratio = d1 / d2;
                                    const float dist_ratio = 0.3f;
                                    if (dratio > dist_ratio &&
                                        dratio < (1.0f /
                                                  dist_ratio))
                                    { // both fingers moved roughly the same distance, so it's a double-finger swipe!
                                        float xavg = 0.5f * (x1 + x2);
                                        float yavg = 0.5f * (y1 + y2);
                                        if (xavg > 0 && xavg > std::abs(yavg))
                                        {
                                            last_input = input_event(get_key_event_from_string(
                                                                         get_option<std::string>("ANDROID_2_SWIPE_LEFT_KEY")),
                                                                     input_event_t::keyboard_char);
                                        }
                                        else if (xavg < 0 && -xavg > std::abs(yavg))
                                        {
                                            last_input = input_event(get_key_event_from_string(
                                                                         get_option<std::string>("ANDROID_2_SWIPE_RIGHT_KEY")),
                                                                     input_event_t::keyboard_char);
                                        }
                                        else if (yavg > 0 && yavg > std::abs(xavg))
                                        {
                                            last_input = input_event(get_key_event_from_string(
                                                                         get_option<std::string>("ANDROID_2_SWIPE_DOWN_KEY")),
                                                                     input_event_t::keyboard_char);
                                        }
                                        else
                                        {
                                            last_input = input_event(get_key_event_from_string(
                                                                         get_option<std::string>("ANDROID_2_SWIPE_UP_KEY")),
                                                                     input_event_t::keyboard_char);
                                        }
                                    }
                                }
                                else
                                {
                                    ...
                                    {
                                        last_input = input_event(get_key_event_from_string(
                                                                     get_option<std::string>("ANDROID_PINCH_IN_KEY")),
                                                                 input_event_t::keyboard_char);
                                    }
                                    else if (curr_dist > down_dist / zoom_ratio)
                                    {
                                        last_input = input_event(get_key_event_from_string(
                                                                     get_option<std::string>("ANDROID_PINCH_OUT_KEY")),
                                                                 input_event_t::keyboard_char);
                                    }
                                }
                            }
...
static void init_term_size_and_scaling_factor()
{
    ...
    if (scaling_factor > 1)
    {
        ...
        get_options().get_option("TERMINAL_X").setValue(std::max(FULL_SCREEN_WIDTH * scaling_factor, terminal.x));
        get_options().get_option("TERMINAL_Y").setValue(std::max(FULL_SCREEN_HEIGHT * scaling_factor, terminal.y));
