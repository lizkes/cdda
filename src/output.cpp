                     ...
                     const std::string &title, const std::string &text)
{
    ...
    ui.on_redraw([&](const ui_adaptor &) {
        ...
        scrollbar().offset_x(width - 1).offset_y(text2.y).content_size(lines.size()).viewport_pos(std::min(beg_line, max_beg_line)).viewport_size(text_h).apply(w);
        wnoutrefresh(w);
    });
...
bool query_yn(const std::string &text)
{
    ...
    return query_popup()
               ...
               .context("YESNO")
               .message(force_uc && !is_keycode_mode_supported() ? pgettext("query_yn", "%s (Case Sensitive)") : pgettext("query_yn", "%s"), text)
                            ...
                            item_filter_type type)
{
    ...
    const std::array<std::string, 3> intros = {{_("Type part of an item's name to filter it."),
    ...
    const auto redraw = [&]() {
        ...
        draw_scrollbar(win, *data.ptr_selected, height, folded.size(),
                       point(data.scrollbar_left ? 0 : getmaxx(win) - 1,
                             (data.without_border && data.use_full_win ? 0 : 1)),
                       BORDER_COLOR, true);
...
std::string word_rewrap(const std::string &in, int width, const uint32_t split)
{
    ...
    for (int j = 0, x = 0; j < static_cast<int>(in.size());)
    {
        ...
        if (uc == split || uc >= 0x2E80)
        { // param split (default ' ') or CJK characters
            if (x <= width)
            {
...
void scrolling_text_view::draw(const nc_color &base_color)
{
    ...
    if (max_offset() > 0)
    {
        scrollbar().content_size(text_.size()).viewport_pos(offset_).viewport_size(height).scroll_to_last(false).apply(w_);
    }
    else
    {
                              ...
                              const std::string &p_sType)
{
    if (get_option<bool>("ANIMATION_SCT"))
    {
        ...
        else
        {
            ...
            {
                p_oDir = (one_in(2)) ? (iso_mode ? direction::EAST : direction::NORTHEAST) : (iso_mode ? direction::SOUTH : direction::SOUTHEAST);
            }
            else if (p_oDir == (iso_mode ? direction::NORTHWEST : direction::WEST))
            {
                p_oDir = (one_in(2)) ? (iso_mode ? direction::NORTH : direction::NORTHWEST) : (iso_mode ? direction::WEST : direction::SOUTHWEST);
            }
        }
...
int scrollingcombattext::cSCT::getPosX() const
{
    if (getStep() > 0)
    {
        ...
        return pos.x + iDirOffset + (dir.x * ((sType == "hp") ? (getStepOffset() + 1) : (getStepOffset() * (iso_mode ? 2 : 1) + getStep())));
    }
...
int scrollingcombattext::cSCT::getPosY() const
{
    if (getStep() > 0)
    {
        ...
        return pos.y + iDirOffset + (dir.y * ((iso_mode && sType == "hp") ? (getStepOffset() + 1) : (getStepOffset() * (iso_mode ? 2 : 1) + getStep())));
    }
