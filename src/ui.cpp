...
void uilist::inputfilter()
{
    ...
    filter_popup->context(ctxt).text(filter).ignore_custom_actions(false).max_length(256).window(window, point(4, w_height - 1), w_width - 4);
    do
    {
        ui_manager::redraw();
...
void uilist::show()
{
    ...
    for (int fei = vshift, si = 0; si < vmax; fei++, si++)
    {
        ...
        {
            int ei = fentries[fei];
            nc_color co = (ei == selected ? hilight_color : (entries[ei].enabled || entries[ei].force_color ? entries[ei].text_color : disabled_color));
            ...
            if (pad_size > 3)
            {
                ...
                // to be used.
                const utf8_wrapper entry = utf8_wrapper(ei == selected ? remove_color_tags(entries[ei].txt) : entries[ei].txt);
                int x = pad_left + 4;
                int y = estart + si;
                ...
                {
                    const utf8_wrapper centry = utf8_wrapper(ei == selected ? remove_color_tags(entries[ei].ctxt) : entries[ei].ctxt);
...
void uilist::query(bool loop, int timeout)
{
    ...
    {
        if (entry.enabled && entry.hotkey.has_value() && entry.hotkey.value() != input_event())
        {
