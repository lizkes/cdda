                                                                      ...
                                                                      const item *selected_clothing)
{
    ...
    for (const bodypart_id &bp : p.get_all_body_parts(get_body_part_flags::sorted))
    {
        ...
        {
            if (std::find(bps.begin(), bps.end(), std::pair<bodypart_id, bool>(bp->opposite_part.id(), true)) == bps.end())
            {
                // only add one
                               ...
                               const item *const selected_clothing) const
{
    ...
    for (int i = 0; i < height; ++i)
    {
        ...
        nc_color limb_color = thisline == line ? (highlighted ? h_green : h_light_gray) : (highlighted ? c_green : c_light_gray);
    ...
    if (draw_scrollbar)
    {
        scrollbar().offset_x(width).offset_y(1).content_size(bps.size()).viewport_pos(firstline).viewport_size(height).scroll_to_last(false).apply(win);
    }
}
                                               ...
                                               bool combine)
{
    ...
    case bp_leg_r:
        ...
        s += swim_cost_text((eff_encumbrance / 10) * (50 - p.get_skill_level(skill_swimming) * 2) / 2);
                           ...
                           const player &you, const unsigned int line, const player_display_tab curtab)
{
    ...
    mvwprintz(w_stats, point(25 - utf8_width(io::enum_to_string(you.my_blood_type) + (you.blood_rh_factor ? "+" : "-")), 8),
              c_light_gray,
                             ...
                             const std::vector<trait_id> &traitslist)
{
    ...
    if (line < traitslist.size())
    {
        ...
        // NOLINTNEXTLINE(cata-use-named-point-constants)
        fold_and_print(w_info, point(1, 0), FULL_SCREEN_WIDTH - 2, c_light_gray, string_format("%s: %s", colorize(mdata.name(), mdata.get_display_color()), traitslist[line]->desc()));
    }
    wnoutrefresh(w_info);
}
                             ...
                             const std::vector<bionic> &bionicslist, const size_t bionics_win_size_y)
{
    ...
    else if (line >= bionicslist.size() - half_y)
    { // near the bottom
                            ...
                            const size_t skill_win_size_y)
{
    ...
    {
        min = (skillslist.size() < skill_win_size_y - 1 ? 0 : skillslist.size() - skill_win_size_y + 1);
    }
    else
    {
        min = line - half_y;
                     ...
                     const std::string &race, const input_context &ctxt)
{
    ...
    right_print(w_tip, 0, 1, c_light_gray, string_format(_("[<color_yellow>%s</color>]"), ctxt.get_desc("HELP_KEYBINDINGS")));
...
void player::disp_info()
{
    ...
    if (bmi < character_weight_category::underweight)
    {
        ...
        if (bmi < character_weight_category::underweight)
        {
            ...
            starvation_text += std::string(_("Intelligence")) + " -" + string_format("%2.0f%%", str_penalty * 50.0f);
        }
    ...
    ui_proficiencies.on_screen_resize([&](ui_adaptor &ui_proficiencies) {
        ...
        proficiency_win_size_y = std::min(display_proficiencies().size(),
                                          static_cast<size_t>(maxy - (infooffsetybottom + effect_win_size_y))) +
                                 1;
