                                                 ...
                                                 const cata::optional<tripoint> &loc)
{
    ...
    if (!loc)
    {
        ...
        allowed_volume = f.workbench->allowed_volume;
                                                                ...
                                                                *loc)
                                                            .part_with_feature("WORKBENCH", true))
    {
        // Vehicle workbench
                                               ...
                                               bool clear_path)
{
    ...
    if (cached_moves == moves && radius_mem == radius && cached_time == calendar::turn && cached_position == inv_pos)
    {
...
void Character::complete_craft(item &craft, const cata::optional<tripoint> &loc)
{
    ...
    for (item &newit : newits)
    {
        ...
        else if (food_contained.is_food() && !food_contained.has_flag(flag_NUTRIENT_OVERRIDE))
        {
            ...
            { // If a canned/contained item was crafted…
            ...
            { // Otherwise, the item is already stacked so we need to divide by batch size.
                                                           ...
                                                           const std::function<bool(const item &)> &filter, bool player_inv)
{
            ...
            if (player_inv)
            {
                ...
                if (!found &&
                    amount_of(type, false, std::numeric_limits<int>::max(), filter) +
                            map_inv.amount_of(type, false, std::numeric_limits<int>::max(), filter) >=
                        count)
                {
                    mixed.push_back(component);
                }
    ...
    if (player_has.size() + map_has.size() + mixed.size() == 1)
    { // Only 1 choice
    ...
    { // Let the player pick which component they want to use
        uilist cmenu;
        ...
        { // Index 0-(map_has.size()-1)
            std::string tmpStr = string_format(_("%s (%d/%d nearby)"),
                                               ...
                                               (map_ha.count * batch),
                                               item::count_by_charges(map_ha.type) ? map_inv.charges_of(map_ha.type, INT_MAX, filter) : map_inv.amount_of(map_ha.type, false, INT_MAX, filter));
        ...
        { // Index map_has.size()-(map_has.size()+player_has.size()-1)
            std::string tmpStr = string_format(_("%s (%d/%d on person)"),
                                               ...
                                               item::count_by_charges(player_ha.type) ? charges_of(player_ha.type, INT_MAX, filter) : amount_of(player_ha.type, false, INT_MAX, filter));
            cmenu.addentry(tmpStr);
        }
        for (auto &component : mixed)
        {
            // Index player_has.size()-(map_has.size()+player_has.size()+mixed.size()-1)
            int available = item::count_by_charges(component.type) ? map_inv.charges_of(component.type, INT_MAX, filter) +
                                         ...
                                         const tripoint &origin, int radius)
{
    ...
    int real_count = (selected_comp.count > 0) ? selected_comp.count * batch : std::abs(selected_comp.count);
                                 ...
                                 const std::function<int(int)> &charges_required_modifier)
{
    ...
    { // Variety of options, list them and pick one
        ...
        if (tmenu.ret < 0 || static_cast<size_t>(tmenu.ret) >= map_has.size() + player_has.size() + both_has.size())
        {
            selected.use_from = usage_from::cancel;
            return selected;
        }
...
ret_val<bool> Character::can_disassemble(const item &obj, const read_only_visitable &inv) const
{
    ...
    const recipe &r = recipe_dictionary::get_uncraft((obj.typeId() == itype_disassembly) ? obj.components.front().typeId() : obj.typeId());
    ...
    if (!obj.is_ammo())
    { //we get ammo quantity to disassemble later on
...
bool Character::disassemble(item_location target, bool interactive)
{
    ...
    const auto &r = (obj.typeId() == itype_disassembly) ? obj.get_making() : recipe_dictionary::get_uncraft(obj.typeId());
...
void Character::complete_disassemble(item_location target)
{
    ...
    const recipe &next_recipe = recipe_dictionary::get_uncraft((next_item.typeId() ==
                                                                itype_disassembly)
                                                                   ? next_item.components.front().typeId()
                                                                   : next_item.typeId());
...
{
    if (newit.made_of(phase_id::LIQUID) && p.is_player())
