        ...
        return std::any_of(allowed_template.begin(), allowed_template.end(),
                           [&e](const itype_id itid) {
                               return e.typeId() == itid;
                                                       ...
                                                       _("Introduce a compatible template."), PICKUP_RANGE, _("You don't have any usable templates.\n\nCompatible templates are: ") + name_list);
    ...
    private:
        ...
        bool do_deposit_money()
        {
            ...
            const int amount = prompt_for_amount(ngettext(
                                                     ...
                                                     "Deposit how much?  Max: %d cents.  (0 to cancel) ", money),
                                                 money);
        ...
        bool do_withdraw_money()
        {
            ...
            const int amount = prompt_for_amount(ngettext(
                                                     ...
                                                     "Withdraw how much?  Max: %d cents.  (0 to cancel) ", u.cash),
                                                 u.cash);
...
void iexamine::cardreader(player &p, const tripoint &examp)
{
    bool open = false;
    map &here = get_map();
    itype_id card_type = (here.ter(examp) == t_card_science ? itype_id_science : here.ter(examp) == t_card_military ? itype_id_military : itype_id_industrial);
...
static std::pair<itype_id, const deploy_tent_actor *> find_tent_itype(const furn_str_id &id)
{
    ...
    if (item::type_is_defined(iid))
    {
        ...
        {
            const deploy_tent_actor *actor = dynamic_cast<const deploy_tent_actor *>(pair.second.get_actor_ptr());
            if (!actor)
            {
                continue;
            }
...
void iexamine::safe(player &guy, const tripoint &examp)
{
    ...
    else if (query_yn(_("Attempt to crack the safe?")))
    {
        ...
        time_duration time_base = std::max(150_minutes - 20_minutes * (guy.get_skill_level(skill_traps) - 3) - 10_minutes * (guy.get_per() - 8), 30_minutes);
...
void iexamine::door_peephole(player &p, const tripoint &examp)
{
    ...
    const int choice = uilist(_("Do what with the door?"), {_("Peek through peephole."),
                                                            _("Open door.")});
    if (choice == 0)
    {
        // Peek
                           ...
                           bool auto_forage = false)
{
    ...
    if (p.is_player() && !auto_forage &&
        !query_yn(_("Pick %s?"), furn ? here.furnname(examp) : here.tername(examp)))
    {
        iexamine::none(p, examp);
        return false;
    }
...
void iexamine::harvest_plant(player &p, const tripoint &examp, bool from_activity)
{
    ...
    { // Generic seed, use the seed item data
...
void iexamine::kiln_empty(player &p, const tripoint &examp)
{
    ...
    for (const item &i : items)
    {
        ...
        {
            add_msg(m_bad, _("This kiln contains %s, which can't be made into charcoal!"), i.tname(1, false));
            return;
        }
    }
...
void iexamine::fvat_empty(player &p, const tripoint &examp)
{
    ...
    if (!brew_present)
    {
        ...
        { //Only one brew type was in inventory, so it's automatically used
...
void iexamine::keg(player &p, const tripoint &examp)
{
    ...
    if (!liquid_present || has_container_with_liquid)
    {
        ...
        { //Only one drink type was in inventory, so it's automatically used
...
void trap::examine(const tripoint &examp) const
{
    ...
    if (easy_take_down())
    { // Separated so saying no doesn't trigger the other query.
    ...
    if (query_yn(_("There is a %s there.  Disarm?"), name()))
    {
        ...
        if (roll >= difficulty || difficulty == 0)
        {
            ...
            { // failure might have set off trap
...
void iexamine::sign(player &p, const tripoint &examp)
{
    ...
    if (!tools.empty())
    {
        ...
        std::string query_message = previous_signage_exists ? _("Overwrite the existing message on the sign?") : _("Add a message to the sign?");
        ...
        if (query_yn(query_message))
        {
            ...
            {
                std::string spray_painted_message = previous_signage_exists ? _("You overwrite the previous message on the sign with your graffiti.") : _("You graffiti a message onto the sign.");
...
cata::optional<tripoint> iexamine::getGasPumpByNumber(const tripoint &p, int number)
{
    ...
    {
        const auto t = here.ter(tmp);
        if ((t == ter_str_id("t_gas_pump") || t == ter_str_id("t_gas_pump_a") || t == ter_str_id("t_diesel_pump") || t == ter_str_id("t_diesel_pump_a")) && number == k++)
        {
            return tmp;
        }
    }
...
void iexamine::pay_gas(player &p, const tripoint &examp)
{
    ...
    amenu.addentry(buy_gas, true, 'b', str_to_illiterate_str(string_format(_("Buy %s."), fuelType)));
    ...
    amenu.addentry(0, false, -1, string_format(str_to_illiterate_str(_("Your price per %s unit: ")), fuelType) + format_money(pricePerUnit));
...
static player &best_installer(player &p, player &null_player, int difficulty)
{
    ...
    std::sort(ally_chances.begin(), ally_chances.end(), [&](const std::pair<int, int> &lhs, const std::pair<int, int> &rhs) {
...
void iexamine::autodoc(player &p, const tripoint &examp)
{
    ...
    if (unsafe_usage)
    {
        const std::string &warning_sign = colorize(" /", c_yellow) + colorize("!", c_red) + colorize("\\", c_yellow);
        const std::string &warning = warning_sign + colorize(_(" WARNING: Operator missing"), c_red) + warning_sign;
...
static void smoker_activate(player &p, const tripoint &examp)
{
    ...
    for (item &it : items)
    {
        ...
        {
            add_msg(m_bad, _("This rack contains %s, which can't be smoked!"), it.tname(1, false));
...
void iexamine::quern_examine(player &p, const tripoint &examp)
{
    ...
    if (!active)
    {
        smenu.addentry_desc(1, !empty, 'r',
                            empty ? _("Remove brake and start milling… insert some products for milling first") : _("Remove brake and start milling"),
        ...
        smenu.addentry_desc(2, !full, 'p',
                            full ? _("Insert products for milling… mill is full") : string_format(_("Insert products for milling… remaining capacity is %s %s"), format_volume(remaining_capacity), volume_units_abbr()),
...
void iexamine::smoker_options(player &p, const tripoint &examp)
{
    ...
    if (!active)
    {
        smenu.addentry_desc(1, !empty && has_enough_coal, 'l',
                            empty ? _("Light up and smoke food… insert some food for smoking first") : !has_enough_coal ? string_format(_("Light up and smoke food… need extra %d charges of charcoal"), need_charges - coal_charges) : _("Light up and smoke food"),
                            _("Light up the smoking rack and start smoking.  Smoking will take about 6 hours."));
        if (portable)
        {
            smenu.addentry_desc(2, !full_portable, 'f',
                                full_portable ? _("Insert food for smoking… smoking rack is full") : string_format(_("Insert food for smoking… remaining capacity is %s %s"), format_volume(remaining_capacity_portable), volume_units_abbr()),
            ...
            smenu.addentry_desc(8, !active, 'z',
                                active ? _("You cannot disassemble this smoking rack while it is active!") : _("Disassemble the smoking rack"), "");
        }
        else
        {
            smenu.addentry_desc(2, !full, 'f',
                                full ? _("Insert food for smoking… smoking rack is full") : string_format(_("Insert food for smoking… remaining capacity is %s %s"), format_volume(remaining_capacity), volume_units_abbr()),
                                _("Fill the smoking rack with raw meat, fish or sausages for smoking or fruit or vegetable or smoked meat for drying."));
        }
        ...
        smenu.addentry_desc(3, has_coal_in_inventory, 'r',
                            !has_coal_in_inventory ? _("Reload with charcoal… you don't have any") : _("Reload with charcoal"),
                            string_format(
...
{
    static const std::map<std::string, iexamine_function> function_map = {{{"none", &iexamine::none},
