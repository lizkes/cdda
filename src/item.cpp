...
item &item::ammo_set(const itype_id &ammo, int qty)
{
    ...
    if (ammo_types().count(ammo_type) == 0 && !(magazine_current() && magazine_current()->ammo_types().count(ammo_type)) &&
...
item &item::ammo_unset()
{
    ...
    {
        if (is_money())
        { // charges are set wrong on cash cards.
            charges = 0;
        }
...
namespace
{
    ...
    const std::array<bodypart_str_id, 4> left_side_parts{{bodypart_str_id{"arm_l"},
                                                          bodypart_str_id{"hand_l"},
                                                          bodypart_str_id{"leg_l"},
                                                          bodypart_str_id{"foot_l"}}};

    const std::array<bodypart_str_id, 4> right_side_parts{{bodypart_str_id{"arm_r"},
                      ...
                      bool /* debug */) const
{
    ...
    if (parts->test(iteminfo_parts::BASE_MATERIAL))
    {
        ...
        if (!mat_types.empty())
        {
            const std::string material_list = enumerate_as_string(
                mat_types.begin(), mat_types.end(),
    ...
    if (parts->test(iteminfo_parts::DESCRIPTION))
    {
        ...
        else
        {
            ...
            if (is_craft())
            {
                const std::string desc = (typeId() == itype_disassembly) ? _("This is an in progress disassembly of %s.  "
                                                                             "It is %d percent complete.")
                                                                         : _("This is an in progress %s.  "
                                                                             "It is %d percent complete.");
                     ...
                     const iteminfo_query *parts, int batch, bool debug) const
{
    ...
    if (parts->test(iteminfo_parts::FOOD_ALLERGEN) && player_character.allergy_type(*food_item) != morale_type("morale_null"))
    {
                    ...
                    int /* batch */, bool /* debug */) const
{
    ...
    if (parts->test(iteminfo_parts::GUN_RELOAD_TIME))
    {
        info.emplace_back("GUN", _("Reload time: "),
                          has_flag(flag_RELOAD_ONE) ? _("<num> moves per round") : _("<num> moves"),
    ...
    else if (parts->test(iteminfo_parts::GUN_TYPE))
    {
        ...
        {
            info.emplace_back("GUN", _("Type: "), enumerate_as_string(
                                                      types_of_ammo.begin(), types_of_ammo.end(), [](const ammotype &at) {
                                                          return at->name();
                                                      },
                                                      enumeration_conjunction::none));
        }
    ...
    {
        info.emplace_back("AMMO", _("Ammunition: "), string_format("<stat>%s</stat>", mod->ammo_data()->nname(mod->ammo_remaining())));
    }
    ...
    if (!gun.valid_mod_locations.empty() && parts->test(iteminfo_parts::DESCRIPTION_GUN_MODS))
    {
        ...
        for (std::pair<const gunmod_location, int> &elem : mod_locations)
        {
            ...
            {
                if (gmod->type->gunmod->location == (elem).first)
                { // if mod for this location
                    if (first_mods)
                    {
                        mod_str += ": ";
                        first_mods = false;
        ...
        if (const islot_armor *t = find_armor_data())
        {
            if (!t->data.empty())
            {
                struct armor_portion_type
                {
                    ...
                    {
                        return encumber == other.encumber && max_encumber == other.max_encumber && coverage == other.coverage;
                    }
                };
                ...
                for (const armor_portion_data &piece : t->data)
                {
                    if (piece.covers.has_value())
                    {
                        for (const bodypart_str_id &covering_id : piece.covers.value())
                        {
                            ...
                            {
                                to_display_data[covering_id] = {covering_id.obj().name_as_heading, {get_encumber(player_character, covering_id), get_encumber(player_character, covering_id, encumber_flags::assume_full), piece.coverage}, true};
                            }
                        }
                    }
                }
                ...
                if (!t->sided)
                {
                    for (const armor_portion_data &piece : t->data)
                    {
                        for (const bodypart_str_id &bp : *piece.covers)
                        {
                            ...
                            if (opposite != bp && covers(bp) && covers(opposite) && to_display_data.at(bp).portion == to_display_data.at(opposite).portion && to_display_data.at(opposite).active)
                            {
                       ...
                       int /*batch*/, bool /*debug*/) const
{
    ...
    if (!rep.empty())
    {
        info.emplace_back("DESCRIPTION", string_format(_("<bold>Repair</bold> using %s."),
                                                       enumerate_as_string(
                                                           rep.begin(), rep.end(), [](const itype_id &e) {
                                                               return nname(e);
                                                           },
                            ...
                            int /*batch*/, bool /*debug*/) const
{
    ...
    const recipe &dis = (typeId() == itype_disassembly) ? get_making() : recipe_dictionary::get_uncraft(typeId());
                       ...
                       bool /*debug*/) const
{
    ...
    if (get_option<bool>("CBM_SLOTS_ENABLED") && parts->test(iteminfo_parts::DESCRIPTION_CBM_SLOTS))
    {
    ...
    if (!fuels.empty())
    {
        ...
        info.push_back(iteminfo("DESCRIPTION",
                                ngettext("* This bionic can produce power from the following fuel: ",
                                         ...
                                         fuel_numb) +
                                    enumerate_as_string(fuels.begin(),
                       ...
                       bool /*debug*/) const
{
    ...
    if ((player_character.get_skill_level(skill_melee) > 2 &&
         (dmg_bash || dmg_cut || dmg_stab || type->m_to_hit > 0)) ||
        debug_mode)
    {
                      ...
                      bool debug) const
{
    ...
    if (has_flag(flag_LEAK_DAM) && has_flag(flag_RADIOACTIVE) && damage() > 0 && parts->test(iteminfo_parts::DESCRIPTION_RADIOACTIVITY_DAMAGED))
    {
        info.push_back(iteminfo("DESCRIPTION",
    ...
    std::vector<const itype *> holsters = Item_factory::find([this](const itype &e) {
        ...
        const holster_actor *ptr = dynamic_cast<const holster_actor *>(e.get_use("holster")->get_actor_ptr());
    ...
    if (parts->test(iteminfo_parts::DESCRIPTION_ACTIVATABLE_TRANSFORMATION))
    {
        ...
        {
            const delayed_transform_iuse *tt = dynamic_cast<const delayed_transform_iuse *>(u.second.get_actor_ptr());
            if (tt == nullptr)
            {
                continue;
            }
    ...
    if (item_note != item_vars.end() && parts->test(iteminfo_parts::DESCRIPTION_NOTES))
    {
        ...
        const use_function *use_func =
            item_note_tool != item_vars.end() ? item_controller->find_template(
                                                                   itype_id(item_note_tool->second))
                                                    ->get_use("inscribe")
                                              : nullptr;
        ...
        if (player_character.has_identified(typeId()))
        {
                     ...
                     player_character.get_skill_level(tmp.skill) < tmp.level)
            {
                ret = c_pink;
                         ...
                         *type))
            { // Book can't improve skill anymore, but has more recipes: yellow
                ret = c_yellow;
            }
        }
        else
...
units::mass item::weight(bool, bool integral) const
{
    ...
    else if (magazine_integral() && (!is_magazine() || is_gun()))
    {
        ...
        {
            ret += ammo_remaining() * find_type(ammotype(*ammo_types().begin())->default_ammotype())->weight / PLUTONIUM_CHARGES;
        }
        else if (ammo_data())
        {
...
units::volume item::volume(bool integral) const
{
    ...
    {
        units::quantity<int64_t, units::volume_in_milliliter_tag> num = ret * static_cast<int64_t>(charges);
...
ret_val<bool> item::is_gunmod_compatible(const item &mod) const
{
    ...
    else if ((mod.type->mod->ammo_modifier.empty() || !mod.type->mod->magazine_adaptor.empty()) && (ammo_remaining() > 0 || magazine_current()))
    {
...
item::reload_option::reload_option(const player *who, const item *target, const item *parent,
                                   const item_location &ammo) : who(who), target(target), ammo(ammo), parent(parent)
{
...
void item::reload_option::qty(int val)
{
    ...
    item &ammo_obj = (ammo_in_container || ammo_in_liquid_container) ?
                         ...
                         *ammo->contents.all_items_top(item_pocket::pocket_type::CONTAINER).front()
                                                                     : *ammo;
    ...
    int remaining_capacity = target->is_watertight_container() ? target->get_remaining_capacity_for_liquid(ammo_obj, true) : target->remaining_ammo_capacity();
                       ...
                       const tripoint &pos, const std::function<bool(const item &)> &filter)
{
    ...
    visit_items([&what, &qty, &used, &pos, &del, &filter](item *e, item *parent) {
        ...
        if (e->is_tool())
        {
            if (e->typeId() == what)
            {
                if ((e->is_magazine() || e->magazine_current() != nullptr) && e->ammo_current() != itype_id::NULL_ID())
                {
...
bool item::can_holster(const item &obj, bool) const
{
    ...
    const holster_actor *ptr = dynamic_cast<const holster_actor *>(type->get_use("holster")->get_actor_ptr());
...
void item::calc_temp(const int temp, const float insulation, const time_duration &time_delta)
{
    ...
    const float conductivity_term = 0.0076 * std::pow(to_milliliter(volume()), 2.0 / 3.0) / insulation;
    ...
    {
        // Was solid.
        new_item_temperature = (-temperature_difference * std::exp(-to_turns<int>(time_delta) * conductivity_term / (mass * specific_heat_solid)) + env_temperature);
        ...
        if (new_item_temperature > freezing_temperature + 0.5)
        {
            ...
            extra_time = to_turns<int>(time_delta) - std::log(-temperature_difference / (freezing_temperature - env_temperature)) * (mass * specific_heat_solid / conductivity_term);
            new_specific_energy = completely_frozen_specific_energy + conductivity_term * (env_temperature - freezing_temperature) * extra_time / mass;
            new_item_temperature = freezing_temperature;
            if (new_specific_energy > completely_liquid_specific_energy)
            {
    ...
    {
        // Was liquid.
        new_item_temperature = (-temperature_difference * std::exp(-to_turns<int>(time_delta) * conductivity_term / (mass * specific_heat_liquid)) + env_temperature);
        ...
        if (new_item_temperature < freezing_temperature - 0.5)
        {
            ...
            extra_time = to_turns<int>(time_delta) - std::log(-temperature_difference / (freezing_temperature - env_temperature)) * (mass * specific_heat_liquid / conductivity_term);
            new_specific_energy = completely_liquid_specific_energy + conductivity_term * (env_temperature - freezing_temperature) * extra_time / mass;
            new_item_temperature = freezing_temperature;
            if (new_specific_energy < completely_frozen_specific_energy)
            {
    ...
    else
    {
        ...
        if (new_specific_energy > completely_liquid_specific_energy)
        {
            ...
            new_item_temperature = ((freezing_temperature - env_temperature) * std::exp(-extra_time * conductivity_term / (mass * specific_heat_liquid)) + env_temperature);
            new_specific_energy = (new_item_temperature - freezing_temperature) * specific_heat_liquid +
                                  completely_liquid_specific_energy;
        }
        else if (new_specific_energy < completely_frozen_specific_energy)
        {
            ...
            new_item_temperature = ((freezing_temperature - env_temperature) * std::exp(-extra_time * conductivity_term / (mass * specific_heat_solid)) + env_temperature);
            new_specific_energy = new_item_temperature * specific_heat_solid;
        }
    }
...
bool item::process_UPS(player *carrier, const tripoint & /*pos*/)
{
    ...
    bool has_connected_cable = carrier->has_item_with([](const item &it) {
        return it.active && it.has_flag(flag_CABLE_SPOOL) && (it.get_var("state") == "UPS_link" || it.get_var("state") == "UPS");
    });
...
{
    int moves = recipe_dictionary::get_uncraft(type->get_id()).time_to_craft_moves(guy, recipe_time_flag::ignore_proficiencies);
