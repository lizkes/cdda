...
game::game() : liveview(*liveview_ptr),
               scent_ptr(*this),
...
void game::setup()
{
    ...
    // Weather shift in 30
    weather.nextweather = calendar::start_of_cataclysm + time_duration::from_hours(get_option<int>("INITIAL_TIME")) + 30_minutes;
...
bool game::start_game()
{
    ...
    const start_location &start_loc = u.random_start_location ? scen->random_start_location().obj() : u.start_location.obj();
    ...
    int max_distance, const std::vector<std::string> &omt_search_types)
{
    ...
    for (const std::string &search_type : search_types)
    {
        ...
        for (const tripoint_abs_omt &goal : overmap_buffer.find_all(omt_origin, find_params))
        {
            ...
            static constexpr std::array<units::angle, 4> angles = {{0_degrees, 90_degrees, 180_degrees, 270_degrees}};
...
void game::calc_driving_offset(vehicle *veh)
{
    ...
    offset.x = (std::abs(offset_difference.x) > 1) ? (driving_view_offset.x + offset_sign.x) : offset.x;
    offset.y = (std::abs(offset_difference.y) > 1) ? (driving_view_offset.y + offset_sign.y) : offset.y;
...
bool game::cancel_activity_or_ignore_query(const distraction_type type, const std::string &text)
{
    ...
    const auto &action = query_popup()
                             ...
                             .message(force_uc && !is_keycode_mode_supported() ? pgettext("cancel_activity_or_ignore_query",
...
{
    if (loc->will_spill_if_unsealed() && loc.where() != item_location::type::map && !you.is_wielding(*loc))
    {
                              ...
                              const inventory_item_menu_position position)
{
    ...
    if (u.has_item(oThisItem))
    {
        ...
        const hint_rating rate_drop_item = u.weapon.has_flag(flag_NO_UNWIELD) ? hint_rating::cant : hint_rating::good;
        ...
        do
        {
            ...
            case '+':
                if (!bHPR)
                {
                    ...
                    add_msg(m_info, _("'%s' added to character pickup rules."), oThisItem.tname(1, false));
                }
                break;
            case '-':
                if (bHPR)
                {
                    ...
                    add_msg(m_info, _("'%s' removed from character pickup rules."), oThisItem.tname(1, false));
                }
                break;
            default:
                break;
    ...
    if (!u.getID().is_valid())
    {
        ...
        if (u.in_vehicle)
        {
            if (const cata::optional<vpart_reference> vp = m.veh_at(
                                                                u.pos())
                                                               .part_with_feature("BOARDABLE", true))
            {
...
bool game::save_player_data()
{
    ...
    const bool saved_data = write_to_file(
        playerfile + SAVE_EXTENSION, [&](std::ostream &fout) {
            serialize(fout);
        },
        _("player data"));
    const bool saved_map_memory = write_to_file(
        playerfile + SAVE_EXTENSION_MAP_MEMORY, [&](std::ostream &fout) {
            JsonOut jsout(fout);
            u.serialize_map_memory(jsout);
        },
        _("player map memory"));
    const bool saved_log = write_to_file(
        playerfile + SAVE_EXTENSION_LOG, [&](std::ostream &fout) {
            memorial().save(fout);
        },
        _("player memorial"));
#if defined(__ANDROID__)
    const bool saved_shortcuts = write_to_file(
        playerfile + SAVE_EXTENSION_SHORTCUTS, [&](std::ostream &fout) {
            save_shortcuts(fout);
        },
        _("quick shortcuts"));
#endif
...
bool game::save()
{
    ...
    try
    {
        if (!save_player_data() ||
            ...
            !write_to_file(
                PATH_INFO::world_base_save_path() + "/uistate.json", [&](std::ostream &fout) {
                    JsonOut jsout(fout);
...
void game::write_memorial_file(std::string sLastWords)
{
    ...
    if (get_options().has_option("ENCODING_CONV") && !get_option<bool>("ENCODING_CONV"))
    {
        ...
        std::locale locale{"C"};
        std::replace_copy_if(
            std::begin(u.name), std::begin(u.name) + truncated_name_len,
    ...
    write_to_file(
        text_path_string, [&](std::ostream &fout) {
            memorial().write_text_memorial(fout, sLastWords);
        },
        _("player memorial"));

    write_to_file(
        json_path_string, [&](std::ostream &fout) {
...
static void draw_footsteps(const catacurses::window &window, const tripoint &offset)
{
    ...
    {
        char glyph = '?';
        if (footstep.z != offset.z)
        { // Here z isn't an offset, but a coordinate
        ...
        if (panel.render())
        {
            ...
            if (panel.toggle && panel.render() && h > 0)
            {
                ...
                if (show_panel_adm)
                {
                    ...
                    catacurses::window label = catacurses::newwin(1, panel_name_width, point(sidebar_right ? TERMX - panel.get_width() - panel_name_width - 1 : panel.get_width() + 1, y));
                    werase(label);
...
void game::draw_ter(const tripoint &center, const bool looking, const bool draw_sounds)
{
    ...
    if (!destination_preview.empty() && u.view_offset.z == 0)
    {
        ...
        mvwputch(w_terrain, final_destination.xy() - u.view_offset.xy() + point(POSX - u.posx(), POSY - u.posy()), c_white, 'X');
    }
...
void game::draw_minimap()
{
    ...
    if (!drew_mission)
    {
        double slope = curs2.x() != targ.x() ? static_cast<double>(targ.y() - curs2.y()) / (targ.x() - curs2.x()) : 4;

        if (curs2.x() == targ.x() || std::fabs(slope) > 3.5)
        { // Vertical slope
            if (targ.y() > curs2.y())
            {
...
void game::mon_info(const catacurses::window &w, int hor_padding)
{
    ...
    const std::array<std::string, 8> dir_labels = {{_("North:"), _("NE:"), _("East:"), _("SE:"),
    ...
    for (int j = 8; j >= 0 && pr.y < maxheight; j--)
    {
        ...
        for (const mtype *type : unique_mons[j])
        {
            ...
            if (pr.y < maxheight)
            { // Don't print if we've overflowed
...
void game::mon_info_update()
{
    ...
    const int iProxyDist = (safe_proxy_dist <= 0) ? MAX_VIEW_DISTANCE : safe_proxy_dist;
    ...
    if (newseen > mostseen)
    {
        ...
        if (safe_mode == SAFE_MODE_ON)
        {
            set_safe_mode(SAFE_MODE_STOP);
             ...
             newseen == 0)
    { // Auto-safe mode, but only if it's a new turn
...
void game::knockback(std::vector<tripoint> &traj, int stun, int dam_mult)
{
        ...
        for (size_t i = 1; i < traj.size(); i++)
        {
            ...
            { // oops, we hit a wall!
                ...
                if (stun != 0)
                {
                    ...
                    std::array<bodypart_id, 8> bps = {{bodypart_id("head"),
    ...
    else if (u.pos() == tp)
    {
        if (stun > 0)
        {
            ...
            add_msg(m_bad, ngettext("You were stunned for %d turn!", "You were stunned for %d turns!", stun),
                    stun);
        }
        for (size_t i = 1; i < traj.size(); i++)
        {
            ...
            { // oops, we hit a wall!
                ...
                if (stun != 0)
                {
                    ...
                    {
                        add_msg(m_bad, ngettext("You were stunned AGAIN for %d turn!", "You were stunned AGAIN for %d turns!", force_remaining),
                                force_remaining);
                    }
                    else
                    {
                        add_msg(m_bad, ngettext("You were stunned for %d turn!", "You were stunned for %d turns!", force_remaining),
                                force_remaining);
                    }
...
void game::use_computer(const tripoint &p)
{
    ...
    if (used == nullptr)
    {
        ...
        { //Console without map data
            add_msg(m_bad, _("The console doesn't display anything coherent."));
        }
        else
        {
            dbg(D_ERROR) << "game:use_computer: Tried to use computer at (" << p.x << ", " << p.y << ", " << p.z << ") - none there";
...
bool game::revive_corpse(const tripoint &p, item &it)
{
    ...
    { // if the monster can reanimate has a zombie
...
void game::control_vehicle()
{
    ...
    else if (veh && (veh->avail_part_with_feature(veh_part, "CONTROLS") >= 0 || (veh->avail_part_with_feature(veh_part, "CONTROL_ANIMAL") >= 0 && veh->has_engine_type(fuel_type_animal, false) && veh->has_harnessed_animal())) &&
             u.in_vehicle)
    {
    ...
    { // Start looking for nearby vehicle controls.
                               ...
                               const visibility_variables &cache)
{
    ...
    else
    {
        ...
        for (tmp.z = -OVERMAP_DEPTH; tmp.z <= OVERMAP_HEIGHT; tmp.z++)
        {
            ...
            {
                const int lines = fold_and_print(w_look, point(1, ++line), max_width, c_light_gray,
                                                 tmp.z > lp.z ? _("From above you heard %s") : _("From below you heard %s"), zlev_sound);
                line += lines - 1;
            }
        }
    }
...
void game::zones_manager()
{
    ...
    ui.on_screen_resize([&](ui_adaptor &ui) {
        ...
        offsetX = get_option<std::string>("SIDEBAR_POSITION") != "left" ? TERMX - width : 0;
    ...
    do
    {
        ...
        {
            do
            { // not a loop, just for quick bailing out if canceled
...
void draw_trail(const tripoint &start, const tripoint &end, const bool bDrawX)
{
    ...
    if (bDrawX)
    {
        ...
        {
            mvwputch(g->w_terrain, pts.back().xy() - player_character.view_offset.xy() + point(POSX - player_character.posx(), POSY - player_character.posy()),
                     c_white, sym);
        }
    }
}
...
void game::list_items_monsters()
{
    ...
    std::sort(mons.begin(), mons.end(), [&](const Creature *lhs, const Creature *rhs) {
        ...
        return att_lhs < att_rhs || (att_lhs == att_rhs && rl_dist(u.pos(), lhs->pos()) < rl_dist(u.pos(), rhs->pos()));
    });
...
game::vmenu_ret game::list_monsters(const std::vector<Creature *> &monster_list)
{
    ...
    ui.on_screen_resize([&](ui_adaptor &ui) {
        ...
        else
        {
            ...
            w_monsters = catacurses::newwin(iMaxRows, width - 2, point(offsetX + 1, 1));
            w_monsters_border = catacurses::newwin(iMaxRows + 1, width, point(offsetX, 0));
            w_monster_info = catacurses::newwin(iInfoHeight - 2, width - 2,
                                                point(offsetX + 1, TERMY - iInfoHeight + 1));
            w_monster_info_border = catacurses::newwin(iInfoHeight, width, point(offsetX, TERMY - iInfoHeight));
    ...
    ui.on_redraw([&](const ui_adaptor &) {
        if (!hide_ui)
        {
            ...
            else
            {
                ...
                const int numw = iNumMonster > 999 ? 4 : iNumMonster > 99 ? 3 : iNumMonster > 9 ? 2 : 1;
                ...
                for (int y = 0; y < endY; ++y)
                {
                    ...
                    const int mon_dist = rl_dist(u.pos(), critter->pos());
                    const int numd = mon_dist > 999 ? 4 : mon_dist > 99 ? 3 : mon_dist > 9 ? 2 : 1;
...
static void butcher_submenu(const std::vector<map_stack::iterator> &corpses, int index = -1)
{
    ...
    smenu.addentry_col(static_cast<int>(butcher_type::FIELD_DRESS), enough_light && has_organs,
                       ...
                       enough_light ? (has_organs ? cut_time(butcher_type::FIELD_DRESS) : colorize(_("has no organs"), c_red)) : cannot_see,
                       string_format("%s  %s",
    ...
    smenu.addentry_col(static_cast<int>(butcher_type::SKIN), enough_light && has_skin,
                       's', _("Skin corpse"),
                       enough_light ? (has_skin ? cut_time(butcher_type::SKIN) : colorize(_("has no skin"), c_red)) : cannot_see,
                       string_format("%s  %s",
    ...
    smenu.addentry_col(static_cast<int>(butcher_type::BLEED), enough_light && has_blood,
                       'l', _("Bleed corpse"),
                       enough_light ? (has_blood ? cut_time(butcher_type::BLEED) : colorize(_("has no blood"), c_red)) : cannot_see,
                       string_format("%s  %s",
...
void game::reload(item_location &loc, bool prompt, bool empty)
{
    ...
    item::reload_option opt = u.ammo_location && it->can_reload_with(u.ammo_location->typeId()) ? item::reload_option(&u, it, it, u.ammo_location) : u.select_ammo(*it, prompt, empty);
...
std::vector<std::string> game::get_dangerous_tile(const tripoint &dest_loc) const
{
    ...
    else if (m.has_flag("SHARP", dest_loc) && !m.has_flag("SHARP", u.pos()) && !(u.in_vehicle || m.veh_at(dest_loc)) &&
...
point game::place_player(const tripoint &dest_loc)
{
    ...
    if (m.has_flag("ROUGH", dest_loc) && (!u.in_vehicle) && (!u.is_mounted()))
    {
        if (one_in(5) && u.get_armor_bash(bodypart_id("foot_l")) < rng(2, 5))
        {
            add_msg(m_bad, _("You hurt your left foot on the %s!"),
                    m.has_flag_ter("ROUGH", dest_loc) ? m.tername(dest_loc) : m.furnname(dest_loc));
    ...
    if (m.has_flag("SHARP", dest_loc) && !one_in(3) && !x_in_y(1 + u.dex_cur / 2.0, 40) &&
        (!u.in_vehicle && !m.veh_at(dest_loc)) && (!u.has_trait(trait_PARKOUR) || one_in(4)) && (u.has_trait(trait_THICKSKIN) ? !one_in(8) : true))
    {
        if (u.is_mounted())
        {
        ...
        else
        {
            ...
            if (u.deal_damage(nullptr, bp, damage_instance(damage_type::CUT, rng(1, 10))).total_damage() > 0)
            {
                //~ 1$s - bodypart name in accusative, 2$s is terrain name.
                add_msg(m_bad, _("You cut your %1$s on the %2$s!"),
                        body_part_name_accusative(bp),
                        m.has_flag_ter("SHARP", dest_loc) ? m.tername(dest_loc) : m.furnname(dest_loc));
    ...
    if ((vp1.part_with_feature("CONTROL_ANIMAL", true) ||
         vp1.part_with_feature("CONTROLS", true)) &&
        u.in_vehicle && !u.is_mounted())
    {
...
bool game::phasing_move(const tripoint &dest_loc, const bool via_ramp)
{
           ...
           (critter_at(dest) != nullptr && tunneldist > 0))
    {
        ...
        if (tunneldist * 250_kJ >
            u.get_power_level())
        { //oops, not enough energy! Tunneling costs 250 bionic power per impassable tile
...
bool game::grabbed_furn_move(const tripoint &dp)
{
    ...
    if (shifting_furniture)
    {
        ...
        { // we pushed furniture out of reach
...
void game::vertical_move(int movez, bool force, bool peeking)
{
    ...
    if (!here.has_zlevels())
    {
        ...
        for (monster &critter : all_monsters())
        {
            ...
            if (turns < 10 && coming_to_stairs.size() < 8 && critter.will_reach(to.xy()) && !slippedpast)
            {
...
void game::display_reachability_zones()
{
    if (use_tiles)
    {
        ...
        if (display_overlay_state(ACTION_DISPLAY_REACHABILITY_ZONES))
        {
            ...
            { // user cancelled selection, toggle overlay off
                display_toggle_overlay(ACTION_DISPLAY_REACHABILITY_ZONES);
            }
        }
    }
...
void game::quickload()
{
    ...
    if (active_world->save_exists(save_t::from_player_name(u.name)))
    {
        ...
        { // See if we need to reload anything
            MAPBUFFER.reset();
            overmap_buffer.clear();
            try
            {
...
void game::start_calendar()
{
    ...
    calendar::start_of_game = calendar::turn_zero + 1_hours * scen->initial_hour() + 1_days * scen->initial_day() + get_option<int>("SEASON_LENGTH") * 1_days * scen->initial_season() + 4 * get_option<int>("SEASON_LENGTH") * 1_days * (scen->initial_year() - 1) + 1_days * get_option<int>("SPAWN_DELAY");
