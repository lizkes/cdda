...
namespace io
{
    ...
    std::string enum_to_string<item_pocket::pocket_type>(item_pocket::pocket_type data)
    {
        ...
        case item_pocket::pocket_type::CONTAINER:
            return "CONTAINER";
        case item_pocket::pocket_type::MAGAZINE:
            return "MAGAZINE";
        case item_pocket::pocket_type::MAGAZINE_WELL:
            return "MAGAZINE_WELL";
        case item_pocket::pocket_type::MOD:
            return "MOD";
        case item_pocket::pocket_type::CORPSE:
            return "CORPSE";
        case item_pocket::pocket_type::SOFTWARE:
            return "SOFTWARE";
        case item_pocket::pocket_type::MIGRATION:
            return "MIGRATION";
        case item_pocket::pocket_type::LAST:
            break;
        }
...
void item_pocket::set_item_defaults()
{
    for (item &contained_item : contents)
    {
        ...
        { //Contents are batteries or food
...
{
    const ret_val<item_pocket::contain_code> ret = !is_standard_type() ? ret_val<item_pocket::contain_code>::make_success() : can_contain(it);
    if (ret.success())
    {
        contents.push_back(it);
    }
...
void item_pocket::favorite_settings::info(std::vector<iteminfo> &info) const
{
    ...
    info.push_back(iteminfo("BASE", string_format(_("Item Whitelist: %s"),
                                                  item_whitelist.empty() ? _("(empty)") : enumerate_as_string(item_whitelist.begin(), item_whitelist.end(), [](const itype_id &id) {
                                                      return id->nname(1);
                                                  }))));
    info.push_back(iteminfo("BASE", string_format(_("Item Blacklist: %s"),
                                                  item_blacklist.empty() ? _("(empty)") : enumerate_as_string(item_blacklist.begin(), item_blacklist.end(), [](const itype_id &id) {
                                                      return id->nname(1);
                                                  }))));
