...
void weed_msg(player &p)
{
    ...
    if (howhigh > 12_minutes && one_in(7))
    {
        ...
        {
        case 0:
            p.add_msg_if_player("%s", SNIPPET.random_from_category("weed_Freakazoid_1").value_or(translation()));
            return;
        case 1:
            p.add_msg_if_player("%s", SNIPPET.random_from_category("weed_Simpsons_1").value_or(translation()));
            p.mod_hunger(2);
            return;
        case 2:
            if (smarts > 8)
            {
                p.add_msg_if_player("%s", SNIPPET.random_from_category("weed_Timothy_Leary").value_or(translation()));
            }
            else if (smarts < 3)
            {
            ...
            return;
        case 3:
            p.add_msg_if_player("%s", SNIPPET.random_from_category("weed_Dazed_and_Confused_1").value_or(translation()));
            if (one_in(2))
            {
                p.add_msg_if_player("%s", SNIPPET.random_from_category("weed_Dazed_and_Confused_2").value_or(translation()));
                if (one_in(2))
                {
                    p.add_msg_if_player("%s", SNIPPET.random_from_category("weed_Dazed_and_Confused_3").value_or(translation()));
                }
            }
            return;
        case 4:
            if (p.has_amount(itype_money_bundle, 1))
            { // Half Baked
                p.add_msg_if_player("%s", SNIPPET.random_from_category("weed_Half_Baked_1").value_or(translation()));
                if (one_in(2))
                {
                    p.add_msg_if_player("%s", SNIPPET.random_from_category("weed_Half_Baked_2").value_or(translation()));
                    if (one_in(3))
                    {
                        p.add_msg_if_player("%s", SNIPPET.random_from_category("weed_Half_Baked_3").value_or(translation()));
                    }
                }
            }
            ...
            {
                p.add_msg_if_player("%s", SNIPPET.random_from_category("weed_Big_Lebowski").value_or(translation()));
            }
            return;
        case 5:
            p.add_msg_if_player("%s", SNIPPET.random_from_category("weed_Mitch_Hedberg").value_or(translation()));
        default:
            return;
        }
    }
    else if (howhigh > 10_minutes && one_in(5))
    {
        ...
        {
        case 0:
            p.add_msg_if_player("%s", SNIPPET.random_from_category("weed_Bob_Marley").value_or(translation()));
            return;
        case 1:
            p.add_msg_if_player("%s", SNIPPET.random_from_category("weed_Freakazoid_2").value_or(translation()));
            return;
        case 2:
            p.add_msg_if_player("%s", SNIPPET.random_from_category("weed_Simpsons_2").value_or(translation()));
            if (smarts > 2 && one_in(2))
            {
                p.add_msg_if_player("%s", SNIPPET.random_from_category("weed_Simpsons_3").value_or(translation()));
            }
            return;
        case 3:
            p.add_msg_if_player("%s", SNIPPET.random_from_category("weed_Bill_Hicks").value_or(translation()));
            return;
        case 4:
            p.add_msg_if_player("%s", SNIPPET.random_from_category("weed_Steve_Martin_1").value_or(translation()));
            if (one_in(4))
            {
                p.add_msg_if_player("%s", SNIPPET.random_from_category("weed_Steve_Martin_2").value_or(translation()));
            }
            if (one_in(4))
            {
                p.add_msg_if_player("%s", SNIPPET.random_from_category("weed_Steve_Martin_3").value_or(translation()));
            }
            if (one_in(4))
            {
                p.add_msg_if_player("%s", SNIPPET.random_from_category("weed_Steve_Martin_4").value_or(translation()));
            }
            if (one_in(4))
            {
                p.add_msg_if_player("%s", SNIPPET.random_from_category("weed_Steve_Martin_5").value_or(translation()));
            }
            if (smarts > 2)
            {
                p.add_msg_if_player("%s", SNIPPET.random_from_category("weed_Steve_Martin_6").value_or(translation()));
            }
            return;
        case 5:
        default:
    ...
    else if (howhigh > 5_minutes && one_in(3))
    {
        ...
        {
        case 0:
            p.add_msg_if_player("%s", SNIPPET.random_from_category("weed_Cheech_and_Chong").value_or(translation()));
            return;
        case 1:
            p.add_msg_if_player("%s", SNIPPET.random_from_category("weed_Real_Life_1").value_or(translation()));
            p.mod_hunger(4);
            if (p.has_trait(trait_VEGETARIAN))
            {
                p.add_msg_if_player("%s", SNIPPET.random_from_category("weed_Real_Life_2").value_or(translation()));
            }
            else if (p.has_trait(trait_LACTOSE))
            {
                p.add_msg_if_player("%s", SNIPPET.random_from_category("weed_Real_Life_3").value_or(translation()));
            }
            return;
        case 2:
            p.add_msg_if_player("%s", SNIPPET.random_from_category("weed_Dazed_and_Confused_4").value_or(translation()));
            return;
        case 3:
            p.add_msg_if_player("%s", SNIPPET.random_from_category("weed_Half_Baked_4").value_or(translation()));
            return;
        case 4:
            // re-roll
            weed_msg(p);
...
double effect::get_percentage(const std::string &arg, int val, bool reduced) const
{
    ...
    if (bot_base != 0 || bot_scale != 0)
    {
        ...
        else
        {
            ...
            ret = 100 * static_cast<double>(top_base + top_scale) / static_cast<double>(bot_base + bot_scale);
        }
    }
    else
    {
