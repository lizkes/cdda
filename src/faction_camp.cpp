...
namespace base_camps
{
    ...
    std::map<std::string, miss_data> miss_info = {{{"_faction_upgrade_camp", {"Upgrade Camp", to_translation("Upgrade camp"), to_translation("Working to expand your camp!\n"), "Recover Ally from Upgrading", to_translation("Recover Ally from Upgrading")}},
                                                   {"_faction_camp_recall", {"Emergency Recall", to_translation("Emergency Recall"), to_translation("Lost in the ether!\n"), "Emergency Recall", to_translation("Emergency Recall")}},
                                                   {"_faction_camp_crafting_", {"Craft Item", to_translation("Craft Item"), to_translation("Busy crafting!\n"), " (Finish) Crafting", to_translation(" (Finish) Crafting")}},
                                                   {"traveling", {"Traveling", to_translation("Traveling"), to_translation("Busy traveling!\n"), "Recall ally from traveling", to_translation("Recall ally from traveling")}},
                                                   {"_faction_camp_gathering", {"Gather Materials", to_translation("Gather Materials"), to_translation("Searching for materials to upgrade the camp.\n"), "Recover Ally from Gathering", to_translation("Recover Ally from Gathering")}},
                                                   {"_faction_camp_firewood", {"Collect Firewood", to_translation("Collect Firewood"), to_translation("Searching for firewood.\n"), "Recover Firewood Gatherers", to_translation("Recover Firewood Gatherers")}},
                                                   {"_faction_camp_menial", {"Menial Labor", to_translation("Menial Labor"), to_translation("Performing menial labor…\n"), "Recover Menial Laborer", to_translation("Recover Menial Laborer")}},
                                                   {"_faction_camp_expansion", {"Expand Base", to_translation("Expand Base"), to_translation("Surveying for expansion…\n"), "Recover Surveyor", to_translation("Recover Surveyor")}},
                                                   {"_faction_camp_cut_log", {"Cut Logs", to_translation("Cut Logs"), to_translation("Cutting logs in the woods…\n"), "Recover Log Cutter", to_translation("Recover Log Cutter")}},
                                                   {"_faction_camp_clearcut", {"Clearcut", to_translation("Clear a forest"), to_translation("Clearing a forest…\n"), "Recover Clearcutter", to_translation("Recover Clearcutter")}},
                                                   {"_faction_camp_hide_site", {"Setup Hide Site", to_translation("Setup Hide Site"), to_translation("Setting up a hide site…\n"), "Recover Hide Setup", to_translation("Recover Hide Setup")}},
                                                   {"_faction_camp_hide_trans", {"Relay Hide Site", to_translation("Relay Hide Site"), to_translation("Transferring gear to a hide site…\n"), "Recover Hide Relay", to_translation("Recover Hide Relay")}},
                                                   {"_faction_camp_foraging", {"Camp Forage", to_translation("Forage for plants"), to_translation("Foraging for edible plants.\n"), "Recover Foragers", to_translation("Recover Foragers")}},
                                                   {"_faction_camp_trapping", {"Trap Small Game", to_translation("Trap Small Game"), to_translation("Trapping Small Game.\n"), "Recover Trappers", to_translation("Recover Trappers")}},
                                                   {"_faction_camp_hunting", {"Hunt Large Animals", to_translation("Hunt Large Animals"), to_translation("Hunting large animals.\n"), "Recover Hunter", to_translation("Recover Hunter")}},
                                                   {"_faction_camp_om_fortifications", {"Construct Map Fort", to_translation("Construct Map Fortifications"), to_translation("Constructing fortifications…\n"), "Finish Map Fort", to_translation("Finish Map Fortifications")}},
                                                   {"_faction_camp_recruit_0", {"Recruit Companions", to_translation("Recruit Companions"), to_translation("Searching for recruits.\n"), "Recover Recruiter", to_translation("Recover Recruiter")}},
                                                   {"_faction_camp_scout_0", {"Scout Mission", to_translation("Scout Mission"), to_translation("Scouting the region.\n"), "Recover Scout", to_translation("Recover Scout")}},
                                                   {"_faction_camp_combat_0", {"Combat Patrol", to_translation("Combat Patrol"), to_translation("Patrolling the region.\n"), "Recover Combat Patrol", to_translation("Recover Combat Patrol")}},
                                                   {"_faction_upgrade_exp_", {" Expansion Upgrade", to_translation(" Expansion Upgrade"), to_translation("Working to upgrade your expansions!\n"), "Recover Ally", to_translation("Recover Ally")}},
                                                   {"_faction_exp_chop_shop_", {" Chop Shop", to_translation(" Chop Shop"), to_translation("Working at the chop shop…\n"), " (Finish) Chop Shop", to_translation(" (Finish) Chop Shop")}},
                                                   {"_faction_exp_kitchen_cooking_", {" Kitchen Cooking", to_translation(" Kitchen Cooking"), to_translation("Working in your kitchen!\n"), " (Finish) Cooking", to_translation(" (Finish) Cooking")}},
                                                   {"_faction_exp_blacksmith_crafting_", {" Blacksmithing", to_translation(" Blacksmithing"), to_translation("Working in your blacksmith shop!\n"), " (Finish) Smithing", to_translation(" (Finish) Smithing")}},
                                                   {"_faction_exp_plow_", {" Plow Fields", to_translation(" Plow Fields"), to_translation("Working to plow your fields!\n"), " (Finish) Plow Fields", to_translation(" (Finish) Plow fields")}},
                                                   {"_faction_exp_plant_", {" Plant Fields", to_translation(" Plant Fields"), to_translation("Working to plant your fields!\n"), " (Finish) Plant Fields", to_translation(" (Finish) Plant Fields")}},
                                                   {"_faction_exp_harvest_", {" Harvest Fields", to_translation(" Harvest Fields"), to_translation("Working to harvest your fields!\n"), " (Finish) Harvest Fields", to_translation(" (Finish) Harvest Fields")}},
                                                   {"_faction_exp_farm_crafting_", {" Farm Crafting", to_translation(" Farm Crafting"), to_translation("Working on your farm!\n"), " (Finish) Crafting", to_translation(" (Finish) Crafting")}}}};
} // namespace base_camps
...
void basecamp::get_available_missions_by_dir(mission_data &mission_key, const point &dir)
{
    ...
    if (dir != base_camps::base_dir)
    {
        ...
        std::sort(upgrades.begin(), upgrades.end(), [](const basecamp_upgrade &p, const basecamp_upgrade &q) -> bool { return p.name.translated_lt(q.name); });
                ...
                if (cur_npc)
                {
                    if (cur_npc->has_job())
                    {
                        for (activity_id &elem : cur_npc->job.get_prioritised_vector())
                        {
                            ...
                            mvwprintz(w_jobs, point(46, start_y), c_light_gray, string_format(_("%s : %s"), test_act.get_verb(), std::to_string(priority)));
                            start_y++;
                        }
                    }
                    else
            ...
            if (cur_npc)
            {
                while (true)
                {
                    ...
                    for (const activity_id &elem : job_vec)
                    {
                        player_activity test_act = player_activity(elem);
                        const int priority = cur_npc->job.get_priority_of_job(elem);
                        smenu.addentry(count, true, MENU_AUTOASSIGN, string_format(_("%s : %s"), test_act.get_verb(), std::to_string(priority)));
                        count++;
                    }
                    smenu.query();
...
void basecamp::start_relay_hide_site()
{
    ...
    if (forest != tripoint_abs_omt(-999, -999, -999))
    {
        ...
        if (!losing_equipment.empty() || !gaining_equipment.empty())
        {
            ...
            int haulage = trips <= 2 ? 0 : std::max(gaining_equipment.size(), losing_equipment.size());
            time_duration build_time = 6_hours;
            time_duration travel_time = companion_travel_time_calc(forest, omt_pos, 0_minutes,
                                                                   trips, haulage);
            time_duration work_time = travel_time + build_time;
...
bool basecamp::distribute_food()
{
    ...
    const auto consume_non_recursive = [&](item &it, item *const container) {
        ...
        const int kcal = it.get_comestible()->default_nutrition.kcal * it.count() * rot_multip(it, container);
        if (kcal <= 0)
        {
        ...
        else
        {
            if (one_in(2))
            {
                popup(_("%s turned to find the hideous black eyes of a giant wasp "
                        "staring back from only a few feet away…"),
                      comp.name);
